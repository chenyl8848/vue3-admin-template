<template>
  <div class="slider">
    <div class="content">
      ...
      <div class="bg-img-div" :class="bgImgClass"></div>
    </div>
...
const refreshBtn = async () => {
  const result: CaptchaResponse = await generateCaptcha(CaptchaType.CONCAT)
  if (result.code === 200) {
    reset()
    currentCaptchaId.value = result.data.id
    bgImgSrc.value = result.data.captcha.backgroundImage
    sliderImgSrc.value = result.data.captcha.backgroundImage
    initConfig(260, 158.64, 48.58, 159, 206)

    bgImgClass.value = 'bgImgClass'
  } else {
...
.sliderImgClass {
  background-position-x: v-bind(movX) !important;
  //transform: translate(v-bind(movX), 0px) !important;
}

.bgImgClass {
  background-image: url(v-bind(bgImgSrc)) !important;
}
